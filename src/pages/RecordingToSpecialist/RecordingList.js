...
const RecordingToSpecialistList = () => {
    ...
    const handleChange = (id) => {
        ...
        putU(`recording_to_specialist/${id}`, {status: 1 - recordings.find(el => el.id === id).status})// Меняет 0 на 1 и наоборот
            .then((resp) => {
    ...
    const loadServices = () => {
        ...
        let endpoint = `recording_to_specialist?page=${page + 1}&limit=${limit}`;
    ...
    const onDelete = (id) => {
        confirm({
            ...
            onConfirm: () => {
                deleteU(`recording_to_specialist/${id}`)
                    .then((resp) => {
                        if (resp.status === 'success') {
                            loadServices();
                        }
    ...
    return (
        <>
            <Helmet>
                <title>Записи к специалисту</title>
            </Helmet>
                        ...
                        isDataLoading ?
                            ...
                            <>
                                <Box sx={{display: 'flex', justifyContent: 'flex-end'}}>
                                    <Box sx={{marginLeft: 2}}>
                                        <Link to="/app/recording_to_specialist/add">
                                                    ...
                                                    <TableBody>
                                                        {recordings?.map((recording) => (
                                                            <TableRow hover key={recording.id}>
                                                                ...
                                                                <TableCell>
                                                                    {specialists.find(el => el.id === recording.specialist_id)?.fio || '---'}
                                                                </TableCell>
                                                                <TableCell>
...
export default RecordingToSpecialistList;
