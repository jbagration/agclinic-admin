...
const PriceSectionAdd = () => {
    ...
    const validate = () => {
        ...
        if (values.value === '') {
            ...
            showAlert('error', "Название раздела не должно быть пустым", 2)
        }
    ...
    const submit = async () => {
        if (validate()) {
            ...
            postU('price/section', data)
                .then((resp) => {
                    if (resp.status === 'success') {
                        showAlert('success', 'Раздел добавлен', 3);
                        clearForm();
                    } else {
...
export default PriceSectionAdd;
